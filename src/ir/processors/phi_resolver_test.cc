...
TEST(PhiResolverTest, ResolvesPhisAfterSimpleBranch) {
  ...
  auto instr_a = std::make_unique<ir::IntUnaryInstr>(
      value_a, common::Int::UnaryOp::kNeg,
      std::make_shared<ir::IntConstant>(common::Int(int64_t{321})));
  auto instr_a_ptr = instr_a.get();
  auto instr_b = std::make_unique<ir::IntCompareInstr>(
      value_i, common::Int::CompareOp::kLss, value_a,
      std::make_shared<ir::IntConstant>(common::Int(int64_t{222})));
