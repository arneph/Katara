...
Block::Block(int64_t number, Func* func) : number_(number), func_(func) {}
...
int64_t Block::number() const { return number_; }

std::string Block::name() const { return name_; }

void Block::set_name(std::string name) { name_ = name; }
...
BlockValue Block::block_value() const { return BlockValue(number_); }
...
void Block::AddInstr(Instr* instr) { InsertInstr(instrs_.size(), instr); }

void Block::InsertInstr(size_t index, Instr* instr) {
  if (index < 0 || index > instrs_.size()) throw "insertion index out of bounds";
  if (instr == nullptr) throw "tried to add nullptr instruction to block";
  if (instr->number_ != -1)
    throw "tried to add instruction to block that is already used elsewhere";
...
void Block::RemoveInstr(int64_t inum) {
  ...
  if (it == func_->instr_lookup_.end()) throw "tried to remove instruction not owned by function";

  RemoveInstr(it->second);
}

void Block::RemoveInstr(Instr* instr) {
  if (instr == nullptr) throw "tried to remove nullptr instruction";
  if (instr->block_ != this) throw "tried to remove instruction not owned by block";

  auto it = std::find(instrs_.begin(), instrs_.end(), instr);
  if (it == instrs_.end()) throw "tried to remove instruction not owned by block";
...
const std::unordered_set<Block*>& Block::parents() const { return parents_; }

const std::unordered_set<Block*>& Block::children() const { return children_; }
...
Block* Block::BranchingParent() const {
  if (!HasBranchingParent()) throw "block has no branching parent";
...
Block* Block::MergingChild() const {
  if (!HasMergingChild()) throw "block has no merging child";
