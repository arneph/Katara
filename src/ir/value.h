...
extern std::vector<ir::Computed> set_to_ordered_vec(const std::unordered_set<ir::Computed>& set);
extern void set_to_stream(const std::unordered_set<ir::Computed>& set, std::stringstream& ss);
...
bool operator==(const BlockValue& lhs, const BlockValue& rhs);
bool operator!=(const BlockValue& lhs, const BlockValue& rhs);
...
struct hash<ir::BlockValue> {
  std::size_t operator()(const ir::BlockValue& b) const { return b.block(); }
};
