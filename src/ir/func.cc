...
Func::Func(int64_t number, Prog *prog)
    : number_(number), prog_(prog) {}
...
const std::vector<Computed>& Func::args() const {
    return args_;
}

std::vector<Type>& Func::result_types() {
    return result_types_;
}

const std::vector<Type>& Func::result_types() const {
    return result_types_;
}

const std::unordered_set<Block *>& Func::blocks() const {
    return blocks_;
}

Block * Func::entry_block() const {
    return entry_block_;
}
...
bool Func::HasBlock(int64_t bnum) const {
    return block_lookup_.count(bnum) > 0;
}
...
void Func::RemoveBlock(int64_t bnum) {
    auto it = block_lookup_.find(bnum);
    if (it == block_lookup_.end())
        throw "tried to remove block not owned by function";
...
void Func::RemoveBlock(Block *block) {
    if (block == nullptr)
        throw "tried to remove nullptr block";
    if (block->func() != this)
        throw "tried to remove block not owned by function";
...
void Func::AddControlFlow(Block *parent, Block *child) {
    if (parent == nullptr)
        throw "tried to add control flow to nullptr block";
    if (parent->func() != this)
        throw "tried to add control flow to block not owned by function";
    if (child == nullptr)
        throw "tried to add control flow to nullptr block";
    if (child->func() != this)
        throw "tried to add control flow to block not owned by function";
...
void Func::RemoveControlFlow(Block *parent, Block *child) {
    if (parent == nullptr)
        throw "tried to remove control flow from nullptr block";
    if (parent->func() != this)
        throw "tried to remove control flow from block not owned "
              "by function";
    if (child == nullptr)
        throw "tried to remove control flow from nullptr block";
    if (child->func() != this)
...
std::string Func::ToString() const {
    ...
    for (auto it : block_lookup_)
        bnums.push_back(it.first);
...
vcg::Graph Func::ToControlFlowGraph() const {
    ...
    for (Block *block : blocks_) {
        ...
        for (Block *child : block->children()) {
            graph.edges().push_back(vcg::Edge(block->number(),
                                              child->number(),
                                              /*is_directed=*/ true));
        }
    }
...
void Func::Link(DomTreeContext &ctx, int64_t v, int64_t w) const {
    ctx.ancestor_[w] = v;
}
...
void Func::Compress(DomTreeContext &ctx, int64_t v) const {
    ...
    if (ctx.sdom_.at(ctx.label_.at(ctx.ancestor_.at(v))) <
        ctx.sdom_.at(ctx.label_.at(v))) {
...
void Func::FindExplicitIDoms(DomTreeContext &ctx) const {
    for (size_t i = 1; i < ctx.tree_order_.size(); i++) {
        ...
        if (ctx.idom_.at(w) !=
            ctx.tree_order_.at(ctx.sdom_.at(w))) {
