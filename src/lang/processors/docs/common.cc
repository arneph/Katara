...
std::string TagsForText(std::string text, TextFormat format, std::string id, std::string classs,
  ...
  if (link.has_value()) {
    ss << "<a href=\"" << link.value().link << "\" "
       ...
       << "} "
       << "document.getElementById('" << link.value().linked_id
...
html::TextFormat FormatForIdent(ast::Ident* ident, types::Info* type_info) {
