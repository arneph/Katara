...
private:
    ValueSpec(std::vector<Ident *> names,
              Expr *type,
...
private:
    FuncDecl(Ident *name,
             ...
             FuncType *func_type,
             BlockStmt *body)
    : kind_(Kind::kFunc), expr_receiver_(nullptr), type_receiver_(nullptr), name_(name),
    type_params_(type_params), func_type_(func_type), body_(body) {}
    
    FuncDecl(ExprReceiver *expr_receiver,
             Ident *name,
             TypeParamList *type_params,
             FuncType *func_type,
             BlockStmt *body)
    : kind_(Kind::kInstanceMethod), expr_receiver_(expr_receiver), type_receiver_(nullptr),
    name_(name), type_params_(type_params), func_type_(func_type), body_(body) {}
    
    FuncDecl(TypeReceiver *type_receiver,
             Ident *name,
             TypeParamList *type_params,
             FuncType *func_type,
             BlockStmt *body)
    : kind_(Kind::kTypeMethod), expr_receiver_(nullptr), type_receiver_(type_receiver), name_(name),
    type_params_(type_params), func_type_(func_type), body_(body) {}
...
private:
    AssignStmt(std::vector<Expr *> lhs,
               pos::pos_t tok_start,
               tokens::Token tok,
               std::vector<Expr *> rhs)
...
private:
    ReturnStmt(pos::pos_t return_start,
...
private:
    ExprSwitchStmt(pos::pos_t switch_start,
                   Stmt *init,
                   Expr *tag,
                   BlockStmt *body)
...
private:
    TypeSwitchStmt(pos::pos_t switch_start,
                   Ident *var,
                   Expr *tag,
                   BlockStmt *body)
...
private:
    CaseClause(pos::pos_t tok_start,
               tokens::Token tok,
...
private:
    BranchStmt(pos::pos_t tok_start,
               tokens::Token tok,
               Ident *label)
...
private:
    CompareExpr(std::vector<Expr *> operands,
                std::vector<pos::pos_t> compare_op_starts,
...
private:
    CallExpr(Expr *func,
             ...
             pos::pos_t r_paren)
    : func_(func),
      l_brack_(l_brack), type_args_(type_args), r_brack_(r_brack),
      l_paren_(l_paren), args_(args), r_paren_(r_paren) {}
...
private:
    FuncType(pos::pos_t func_type_start,
             FieldList *params,
             FieldList *results)
...
private:
    InterfaceType(pos::pos_t interface_start,
                  pos::pos_t l_brace,
                  std::vector<Expr *> embedded_interfaces,
                  std::vector<MethodSpec *> methods,
                  pos::pos_t r_brace)
    : interface_(interface_start), l_brace_(l_brace), embedded_interfaces_(embedded_interfaces),
    methods_(methods), r_brace_(r_brace) {}
...
// MethodSpec ::= ("(" [Ident] ")" | "<" [Ident] ">") Ident FieldList [FieldList] .
...
private:
    MethodSpec(pos::pos_t spec_start,
               tokens::Token kind,
               Ident *instance_type_param,
               Ident *name,
               FieldList *params,
               FieldList *results)
    : spec_start_(spec_start), kind_(kind), instance_type_param_(instance_type_param), name_(name),
...
private:
    StructType(pos::pos_t struct_start,
               pos::pos_t l_brace,
               FieldList *fields,
               pos::pos_t r_brace)
...
private:
    ExprReceiver(pos::pos_t l_paren,
                 Ident *name,
                 tokens::Token pointer,
                 Ident *type_name,
                 std::vector<Ident *> type_parameter_names,
                 pos::pos_t r_paren)
    : l_paren_(l_paren), name_(name), pointer_(pointer), type_name_(type_name),
    type_parameter_names_(type_parameter_names), r_paren_(r_paren) {}
...
private:
    TypeReceiver(pos::pos_t l_brack,
                 Ident *type_name,
                 std::vector<Ident *> type_parameter_names,
                 pos::pos_t r_brack)
    : l_brack_(l_brack), type_name_(type_name), type_parameter_names_(type_parameter_names),
      r_brack_(r_brack) {}
...
private:
    TypeParamList(pos::pos_t l_angle,
