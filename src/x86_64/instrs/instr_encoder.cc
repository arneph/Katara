...
void InstrEncoder::EncodeOperandSize(Size op_size) {
  if (opcode_ != nullptr) fail("attempted to encode operand mode after opcode");
...
void InstrEncoder::EncodeREX() {
  if (opcode_ != nullptr) fail("attempted to encode REX after opcode");
  if (rex_ != nullptr) {
    return;
  }
...
void InstrEncoder::EncodeOpcode(uint8_t opcode_a) {
  if (opcode_ != nullptr) fail("attempted to encode opcode twice");
...
void InstrEncoder::EncodeOpcodeExt(uint8_t opcode_ext) {
  if (opcode_ == nullptr) fail("attempted to encode reg without opcode");
  if (imm_ != nullptr) fail("attempted to encode reg after imm");
...
void InstrEncoder::EncodeOpcodeReg(const Reg& reg, uint8_t opcode_index, uint8_t lshift) {
  if (opcode_ == nullptr) fail("attempted to encode reg in missing opcode");
  if (imm_ != nullptr) fail("attempted to encode reg after imm");
  if (opcode_index >= 3) fail("opcode index out of range: " + std::to_string(opcode_index));
...
void InstrEncoder::EncodeRM(const RM& rm) {
  if (opcode_ == nullptr) fail("attempted to encode reg without opcode");
  if (sib_ != nullptr || disp_ != nullptr) fail("attempted to encode ModRM twice");
  if (imm_ != nullptr) fail("attempted to encode rm after imm");
  ...
  if (size_ > code_.size()) fail("instruction exceeds code capacity");

  rm.EncodeInModRM_SIB_Disp(rex_, modrm_, sib_, disp_);
}

void InstrEncoder::EncodeImm(const Imm& imm) {
  if (opcode_ == nullptr) fail("attempted to encode imm without opcode");
  if (imm_ != nullptr) fail("attempted to encode imm twice");
