...
uint8_t InstrEncoder::size() const {
    return size_;
}
...
void InstrEncoder::EncodeREX() {
    if (opcode_ != nullptr)
        throw "attempted to encode REX after opcode";
    if (rex_ != nullptr) {
        return;
    }
...
void InstrEncoder::EncodeOpcode(uint8_t opcode_a) {
    if (opcode_ != nullptr)
        throw "attempted to encode opcode twice";
...
void InstrEncoder::EncodeOpcodeExt(uint8_t opcode_ext) {
    ...
    if (imm_ != nullptr)
        throw "attempted to encode reg after imm";
...
void InstrEncoder::EncodeOpcodeReg(Reg *reg,
                                   ...
                                   uint8_t lshift) {
    if (opcode_ == nullptr)
        throw "attempted to encode reg in missing opcode";
    if (imm_ != nullptr)
        throw "attempted to encode reg after imm";
    if (opcode_index >= 3)
        throw "opcode index out of range: " + std::to_string(opcode_index);
    if (lshift > 5)
...
void InstrEncoder::EncodeRM(RM *rm) {
    ...
    if (sib_ != nullptr || disp_ != nullptr)
        throw "attempted to encode ModRM twice";
    if (imm_ != nullptr)
        throw "attempted to encode rm after imm";
