...
uint8_t InstrDecoder::size() const {
    return size_;
}
...
void InstrDecoder::DecodeModRM() {
    ...
    if (sib_ != nullptr ||
        disp_ != nullptr ||
        imm_ != nullptr)
        throw "attemted to decode modRM after decoding "
              "later instruction parts";
...
void InstrDecoder::DecodeSIB() {
    ...
    if (disp_ != nullptr ||
        imm_ != nullptr)
        throw "attemted to decode SIB after decoding "
              "later instruction parts";
...
void InstrDecoder::DecodeDisp(uint8_t disp_size) {
    ...
    if (imm_ != nullptr)
        throw "attemted to decode Disp after decoding "
              "later instruction parts";
...
uint8_t InstrDecoder::DecodeOpcodePart() {
    if (opcode_size_ == 3)
        throw "attempted to decode fourth opcode byte";
    if (modrm_ != nullptr ||
        sib_ != nullptr ||
        disp_ != nullptr ||
        imm_ != nullptr)
        throw "attemted to decode opcode after decoding "
              "later instruction parts";
