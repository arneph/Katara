...
void Parser::ParseFunc() {
    ...
    if (scanner_.token() != Scanner::kArrow)
        throw "expected '=>'";
    scanner_.Next();
...
void Parser::ParseFuncArgs(ir::Func *func) {
    if (scanner_.token() != Scanner::kRoundBracketOpen)
        throw "expected '('";
    scanner_.Next();
...
void Parser::ConnectBlocks(ir::Func *func) {
    for (ir::Block *block : func->blocks()) {
        ...
        if (ir::JumpInstr *jump =
            dynamic_cast<ir::JumpInstr *>(last_instr)) {
            int64_t child_num = jump->destination().block();
            ir::Block *child = func->GetBlock(child_num);
            
            func->AddControlFlow(block, child);
            
        } else if (ir::JumpCondInstr *jump =
                   dynamic_cast<ir::JumpCondInstr *>(last_instr)) {
...
void Parser::ParseBlock(ir::Func *func) {
    ...
    if (scanner_.token() != Scanner::kNewLine)
        throw "expected identifier or new line";
    scanner_.Next();
...
ir::Instr * Parser::ParseInstr() {
    ...
    if (instr_name == "mov") {
        ...
        if (results.size() != 0)
            throw "did not expect results for jump conditional  "
                  "instruction";
        ...
        if (results.size() != 0)
            throw "did not expect results for return instruction";
    ...
    if (instr_name == "mov") {
        ...
        for (ir::Value arg : args) {
            ...
            if (inherited_arg == nullptr)
                throw "expected only inherited value args for "
                      "phi instruction";
        ...
        if (has_instr_type)
            throw "did not expected type for "
                  "jump conditional instruction";
        if (results.size() != 0)
            throw "did not expect results for "
                  "jump conditional instruction";
        if (args.size() != 3)
            throw "expected three operands for "
                  "jump conditional instruction";
...
ir::PhiInstr * Parser::ParsePhiInstr(ir::Computed result) {
    ...
    if (args.size() < 2)
        throw "expected at least two arguments for phi instruction";
    
    return new ir::PhiInstr(result, args);
}

// UnaryALInstr ::= Computed UnaryALOp ':' Type Value NL
ir::UnaryALInstr * Parser::ParseUnaryALInstr(ir::Computed result,
    ...
    if (instr_type != result.type())
        throw "result type does not match type of unary AL instruction";
...
ir::BinaryALInstr * Parser::ParseBinaryALInstr(ir::Computed result,
    ...
    if (scanner_.token() != Scanner::kComma)
        throw "expected ','";
    scanner_.Next();
...
ir::CompareInstr * Parser::ParseCompareInstr(ir::Computed result,
...
ir::JumpCondInstr * Parser::ParseJumpCondInstr() {
    ...
    return new ir::JumpCondInstr(condition,
                                 destination_true,
                                 destination_false);
}
...
//               'call' Value (',' Value)* NL
ir::CallInstr * Parser::ParseCallInstr(
...
ir::Constant Parser::ParseConstant(ir::Type expected_type) {
    if (scanner_.token() == Scanner::kAtSign) {
        if (expected_type != ir::Type::kFunc)
            throw "unexpected '@'";
        scanner_.Next();
    ...
    if (scanner_.token() == Scanner::kColon) {
        ...
        if (expected_type != ir::Type::kUnknown &&
            expected_type != type)
            throw "expected: " + ir::to_string(expected_type) + " "
                  "got: " + ir::to_string(type);
    } else {
        if (expected_type == ir::Type::kUnknown)
            throw "expected ':'";
