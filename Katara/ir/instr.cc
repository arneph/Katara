...
int64_t Instr::number() const {
    return number_;
}

Block * Instr::block() const {
    return block_;
}

Computation::Computation(Computed result)
    : result_(result) {}

Computation::~Computation() {}

Computed Computation::result() const {
    return result_;
}

std::vector<Computed> Computation::DefinedValues() const {
    return std::vector<Computed>{result_};
}

MovInstr::MovInstr(Computed result,
                   Value origin)
    : Computation(result), origin_(origin) {
    if (result.type() != origin.type())
        throw "attempted to create mov instr with "
               "mismatched origin type";
}

MovInstr::~MovInstr() {}

Value MovInstr::origin() const {
    return origin_;
}
...
std::string MovInstr::ToString() const {
    return result().ToStringWithType()
        + " = mov " + origin().ToString();
}

PhiInstr::PhiInstr(Computed result,
                   std::vector<InheritedValue> args)
    : Computation(result), args_(args) {
    for (InheritedValue arg : args) {
        if (result.type() != arg.type())
            throw "attempted to create phi instr with "
                  "mismatched arg type";
    }
}
...
std::vector<InheritedValue> const&
PhiInstr::args() const {
    return args_;
}
...
std::string PhiInstr::ToString() const {
    std::string str = result().ToStringWithType()
        + " = phi ";
...
bool is_unary_al_operation_string(std::string op_str) {
    return op_str == "not"
        || op_str == "neg";
}
...
UnaryALInstr::UnaryALInstr(UnaryALOperation operation,
                           Computed result,
                           Value operand)
    : Computation(result), operation_(operation), operand_(operand) {
    if (!is_integral(result.type()))
        throw "attempted to create unary al instr with "
              "non-integral result type";
...
UnaryALOperation UnaryALInstr::operation() const {
    return operation_;
}

Value UnaryALInstr::operand() const {
    return operand_;
}
...
std::string UnaryALInstr::ToString() const {
    return result().ToStringWithType()
           + " = " + to_string(operation_)
           + ":" + to_string(operand_.type())
           + " " + operand_.ToString();
}
...
BinaryALInstr::BinaryALInstr(BinaryALOperation operation,
                             Computed result,
                             Value operand_a,
                             Value operand_b)
    : Computation(result), operation_(operation),
      operand_a_(operand_a), operand_b_(operand_b) {
    if (!is_integral(result.type()))
        throw "attempted to create binary al instr with "
              "non-integral result type";
    if (result.type() != operand_a.type() ||
        result.type() != operand_b.type())
        throw "attempted to create binary al instr with "
              "mismatched operand type";
}
...
BinaryALOperation BinaryALInstr::operation() const {
    return operation_;
}

Value BinaryALInstr::operand_a() const {
    return operand_a_;
}

Value BinaryALInstr::operand_b() const {
    return operand_b_;
}
...
CompareInstr::CompareInstr(CompareOperation operation,
                           Computed result,
                           Value operand_a,
                           Value operand_b)
    : Computation(result), operation_(operation),
    ...
    if (result.type() != Type::kBool)
        throw "attempted to create compare instr with "
              "non-bool result type";
    if (!is_integral(operand_a.type()))
        throw "attempted to create compare instr with "
              "non-integral operand type";
...
CompareOperation CompareInstr::operation() const {
    return operation_;
}

Value CompareInstr::operand_a() const {
    return operand_a_;
}

Value CompareInstr::operand_b() const {
    return operand_b_;
}
...
JumpInstr::JumpInstr(BlockValue destionation)
    : destination_(destionation) {}

JumpInstr::~JumpInstr() {}

BlockValue JumpInstr::destination() const {
    return destination_;
}

std::vector<Computed> JumpInstr::DefinedValues() const {
    return std::vector<Computed>();
}

std::vector<Computed> JumpInstr::UsedValues() const {
    return std::vector<Computed>();
}

std::string JumpInstr::ToString() const {
    return "jmp " + destination_.ToString();
}

JumpCondInstr::JumpCondInstr(Value condition,
                             BlockValue destination_true,
    ...
    if (condition.type() != Type::kBool)
        throw "attempted to create jump cond instr with "
              "non-bool condition value";
}

JumpCondInstr::~JumpCondInstr() {}

Value JumpCondInstr::condition() const {
    return condition_;
}

BlockValue JumpCondInstr::destination_true() const {
    return destination_true_;
}

BlockValue JumpCondInstr::destination_false() const {
    return destination_false_;
}

std::vector<Computed> JumpCondInstr::DefinedValues() const {
    return std::vector<Computed>();
}
...
std::string JumpCondInstr::ToString() const {
    return "jcc " + condition_.ToString()
           + ", " + destination_true_.ToString()
           + ", " + destination_false_.ToString();
}

CallInstr::CallInstr(Value func,
                     std::vector<Computed> results,
                     std::vector<Value> args)
    : func_(func), results_(results), args_(args) {
    if (func.type() != Type::kFunc)
        throw "attempted to create call instr with "
              "non-function func type";
}

CallInstr::~CallInstr() {}

Value CallInstr::func() const {
    return func_;
}

const std::vector<Computed>& CallInstr::results() const {
    return results_;
}

const std::vector<Value>& CallInstr::args() const {
    return args_;
}

std::vector<Computed> CallInstr::DefinedValues() const {
    return results_;
}
...
ReturnInstr::ReturnInstr(std::vector<Value> args)
    : args_(args) {}

ReturnInstr::~ReturnInstr() {}

const std::vector<Value>& ReturnInstr::args() const {
    return args_;
}

std::vector<Computed> ReturnInstr::DefinedValues() const {
    return std::vector<Computed>();
}
